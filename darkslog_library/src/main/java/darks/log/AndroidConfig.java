...
public final class AndroidConfig {
     ...
     * You can use registerCrashHandler(Callback callback) to register the
     * callback.
     */
     ...
     *                 notify developers. If it's null, it won't call back.
     */
    ...
    private Loader getSdcardConfig(Context ctx) {
        try {
            ...
            if (!file.exists()) {
                file = new File(StorageUtils.getAbsoluteSdcardPath(),
                        configPath);
                if (!file.exists()) {
                    return null;
                }
            ...
            Kernel.logError("Fail to get sdcard config. Cause "
                    + e.getMessage());
        }
        return null;
    }
